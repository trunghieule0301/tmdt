...
function App() {
  ...
  const handleCalculatingTheExpectedDeliveryTime = async () => {
    ...
    calculateTheExpectedDeliveryTime(fromDistrict, fromWard, toDistrict, toWard, service).then(value => {
      console.log(value.leadtime)
      const timestamp = value.leadtime;
      const finalTime = new Date(timestamp * 1000).toLocaleString();
  ...
  return (
    <Box sx={{ width: '80%', maxWidth: 800, margin: 'auto', marginTop: '50px' }}>
      <Typography style={{ marginBottom: '30px' }} variant="h4" component="h5">
        Calculate the expected delivery time (GHN)
      </Typography>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
        ...
        <Grid item xs={4}>
          ...
          >
            {services.filter(value => value.short_name !== '').map((option) => {
              return (
        ...
        <Grid item xs={6}>
          Expected delivery time: {leadtime}
        </Grid>
